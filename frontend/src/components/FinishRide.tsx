...
export function FinishRide({ setFinishRidePanel }: any) {
  return (
    <div className="w-full h-screen bg-gray-100">
      ...
      {/* Footer Buttons */}
      <div className="flex flex-col w-full justify-around items-center bg-white p-4 mt-2">
        {/* we require a payment page in between */}
        <Link
          to="/captain-dashboard"
          className="flex-1 w-full bg-green-500 text-white text-sm font-semibold py-2 rounded-md mx-2 flex items-center justify-center"
        >
          Finish Ride
        </Link>
        <p className="text-red-500 text-sm">
          Finish Ride only if you have completed the payment
        </p>
      </div>
    </div>
  );
